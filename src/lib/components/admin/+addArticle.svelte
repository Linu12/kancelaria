<script>
    ...
    const collectArticleData = () => {
        ...
        languages.forEach(({code}) => {
            ...
            const contentInput = document.getElementById(`article_content_${code}`);
            ...
            if (titleInput?.value) {
                result.title[code] = titleInput.value;
            }

            if (contentInput?.value) {
                result.content[code] = contentInput.value;
            }

            if (pageTitleInput?.value) {
                result.page.title[code] = pageTitleInput.value;
            }

            if (pageDescriptionInput?.value) {
                result.page.description[code] = pageDescriptionInput.value;
            }
        });

        const pageURL = document.getElementById(`article_page_url`);
        result.page.url = pageURL.value
    ...
    <form on:submit|preventDefault={submitArticle} class="max-w-[600px] my-auto space-y-4">
        {#each languages as item, index}
            <div class="border rounded-[5px] shadow-sm">
                <button class="w-full text-left px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-t-lg font-medium text-gray-800"
                ...
                <div class="p-4 bg-white border-t rounded-b-[5px]" class:hidden={openIndex !== index}>
                    <div class="flex flex-col gap-2">
                        <div>
                            <input class="p-2 border outline-none rounded-[5px]" type="text"
                                   name="article_title_{item.code}" id="article_title_{item.code}"
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <textarea class="p-2 border outline-none rounded-[5px] resize-none" type="text"
                                      name="article_content_{item.code}" id="article_content_{item.code}"
                            >{articleData ? articleData.content[item.code] : ""}</textarea>
                        </div>
                        ...
                        <div>
                            <input class="p-2 border outline-none rounded-[5px]" type="text"
                                   name="article_page_title_{item.code}" id="article_page_title_{item.code}"
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <textarea class="p-2 border outline-none rounded-[5px] resize-none" type="text"
                                      ...
                                      id="article_page_description_{item.code}"
                            >{articleData ? articleData.page.description[item.code] : ""}</textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div>
            <span>{$page.url.origin}/info/article/</span>
            <input class="p-[5px] border outline-none rounded-[5px]" type="text"
                   name="article_page_url" id="article_page_url"
                   placeholder="Article Page Url" value={articleData ? articleData.url : ""} required>
        </div>
